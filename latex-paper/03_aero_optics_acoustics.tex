...
A planar wave can be calculated from the set of solutions for duct acoustics, Equation \ref{eqn:02_pressure_solution_duct}, given that $\Psi_m(x_D,y_D)=1$,
\begin{equation}
  \hat{p}(z_D,t) = C_m\exp\left\{j(\omega t \mp k_{zm}^\pm z_D)\right\} \textrm{.}
...
The acoustic field from a speaker may be assumed to be a spherical wave from a pulsating point if the frequency is sufficiently low and the measurement region is far enough away from the source \cite{Randall-1951-9NtPPXPq}, allowing the source to be an effective compact source.
...
Because speakers often have a directivity in their emission, the oscillations can be greatly reduced by using a windowing function in the z-direction, such as a Hann Window \cite{Oppenheim-1999-cfwyTmwq},
\begin{equation}
...
% Just below $\Lambda/Ap=1$ the curve reaches a maximum and begins to drop off quickly due to aperture filtering as previously shown in Figure \ref{fig:03_planar_sample_calc_3}.
...
First, the auto-spectral density of the microphone data was used to calculate the average $p_{RMS}$ at the excitation frequency and the fluctuating pressure strength using Equation \ref{eqn:03_spherical_pressure_rms}.
...
This deviation for the 9 kHz cases is possibily due the speaker no longer acting as a compact source or measurement not occuring in an anechoic environment.
...
The duct modes were solved for at each location through the transition using Equations \ref{eqn:02_wave_xy} and \ref{eqn:02_bc_rigig_wall}.
...
\begin{enumerate}
  ...
  \begin{equation}
    \hat{p}^i(x_D,y_D)=\sum\Psi_m^i(x_D,y_D)\cdot C_m^i
  \end{equation}
...
\end{equation}
  ...
  \begin{equation}
    \hat{p}^{i+1}(x_D,y_D) = \frac{p_t}{p_i}\sum\Psi_M^i(x_D,y_D)\cdot C_m^i\cdot\exp\{\mp j k_{zm}^\pm z\}
  \end{equation}
