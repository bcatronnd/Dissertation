...
showPlots = 0;
nPointsRect = 51;
filename = ['tunnel_acoustic_with_' num2str(tsMach) '_' num2str(fanMode.m) '_' num2str(fanMode.n) '_tsMid.gif'];
frameRate = 15;
plotLocations = [81];

% Code
load(['tunnel_discretization_' num2str(tsMach,'%0.2f') '.mat']);
    % Flip WindTunnel Structure
    WindTunnel.z = fliplr(WindTunnel.z);
    WindTunnel.z = abs(WindTunnel.z-WindTunnel.z(1));
    WindTunnel.area = fliplr(WindTunnel.area);
    WindTunnel.eta = fliplr(WindTunnel.eta);
    WindTunnel.cruciform = [WindTunnel.cruciform(end) WindTunnel.cruciform(1:end-1)];
    WindTunnel.cruciform = fliplr(WindTunnel.cruciform);
    WindTunnel.testsection = [WindTunnel.testsection(end) WindTunnel.testsection(1:end-1)]; 
    WindTunnel.testsection = fliplr(WindTunnel.testsection);
    WindTunnel.mach = fliplr(WindTunnel.mach);
    WindTunnel.temp = fliplr(WindTunnel.temp);
    WindTunnel.c = fliplr(WindTunnel.c);
    WindTunnel.u = fliplr(WindTunnel.u);
    WindTunnel.ptw = fliplr(WindTunnel.ptw);
    WindTunnel.prw = fliplr(WindTunnel.prw);
    WindTunnel.pta = fliplr(WindTunnel.pta);
    WindTunnel.pra = fliplr(WindTunnel.pra);
plotLocations = length(WindTunnel.z)-plotLocations;
...
%         end
        switch floor(WindTunnel.eta(bb))
            case 0 % Circle to Square Transition
                ...
                P = WindTunnel.ptw(bb)*(exp(1i*kzm*WindTunnel.z(bb))*(coeff.*transition_mode.eigvd)')';
            case 1 % Square Duct
                switch floor(WindTunnel.eta(bb-1))
                    case 0 % Transition to Square Grid
                        ...
                        kzm = (WindTunnel.mach(bb)*k0+sqrt(k0^2-(1-WindTunnel.mach(bb)^2)*km.^2))/(1-WindTunnel.mach(bb)^2);
                        P = WindTunnel.ptw(bb)*(exp(1i*kzm*WindTunnel.z(bb))*(coeff.*rectangle_mode.eigv)')';
                    case 1 % Square Duct
                            ...
                            case 1 % Cruciform
                                ...
                                switch WindTunnel.cruciform(bb-1)
                                    case 0 % Transition to Cruciform
                                        ...
                                        P1 = WindTunnel.ptw(bb)*(exp(1i*kzm*WindTunnel.z(bb))*(coeff.*rectangle_mode.eigv1)')';
                                        P2 = P;
                                        ...
                                        P2 = WindTunnel.ptw(bb)*(exp(1i*kzm*WindTunnel.z(bb))*(coeff.*rectangle_mode.eigv2)')';
                                        P3 = P;
                                        ...
                                        P3 = WindTunnel.ptw(bb)*(exp(1i*kzm*WindTunnel.z(bb))*(coeff.*rectangle_mode.eigv3)')';
                                        P4 = P;
                                        ...
                                        P4 = WindTunnel.ptw(bb)*(exp(1i*kzm*WindTunnel.z(bb))*(coeff.*rectangle_mode.eigv4)')';
                                    case 1 % Cruciform
                                        P1(rectangle_mode.mesh.Nodes(1,:)'<0.5) = NaN;
                                        P1(rectangle_mode.mesh.Nodes(2,:)'<0.5) = NaN;
                                        coeff = (reshape(rectangle_mode.eigv1(repmat(~isnan(P1),1,transition_mode.n)),[],transition_mode.n)\P1(~isnan(P1)))';
...
f2.Position = [1 1 5 3];
% f2.Color = 'none';
saveas(f2,['tunnel_acoustic_with_' num2str(tsMach) '_' num2str(fanMode.m) '_' num2str(fanMode.n) '.eps'],'epsc')
% saveas(f2,['tunnel_acoustic_with_' num2str(tsMach) '_' num2str(fanMode.m) '_' num2str(fanMode.n) '.png'],'png')
% exportgraphics(f2,['tunnel_acoustic_with_' num2str(tsMach) '_' num2str(fanMode.m) '_' num2str(fanMode.n) '.png'],'BackgroundColor','none');
