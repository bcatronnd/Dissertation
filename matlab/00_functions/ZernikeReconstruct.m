function phase_screen = ZernikeReconstruct(j,coeff,rho,theta,scheme,epsilon)
%ZERNIKERECONSTRUCT Reconstructs a phase screen
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DESCRIPTION:
% Reconstructs a phase screen with coefficients for the Zernike
% polynomials, j, at the points rho and theta with the given ordering scheme
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INPUTS:
% j [vector] = Index of Zernike polynomial
% coeff [matrix] = Coefficients for each j and time step
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OUTPUTS:
% phase_screen [array] = The reconstructed phase screen for each time step
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% REFERENCES:
...
phase_screen = zeros(size(rho,1),size(rho,2),size(coeff,2));
for aa=1:length(j)
    if strcmp(scheme,'annular')
        if epsilon<0 || epsilon>=1
            error('MATLAB:ZernikeWrongEpsilon','Epsilon Out Of Range.  0<=epsilon<1')
        end
        phase_screen = phase_screen+reshape(coeff(aa,:),1,1,[]).*repmat(ZernikePolynomial(j(aa),rho,theta,scheme,epsilon),1,1,size(coeff,2));
    else
        phase_screen = phase_screen+reshape(coeff(aa,:),1,1,[]).*repmat(ZernikePolynomial(j(aa),rho,theta,scheme),1,1,size(coeff,2));
    end
end
end
